...
@click.option(
    '--fingerprint', '-f', required=True, help='Path to the Golden Template JSON (MANDATORY)'
)
def test(url: Optional[str] = None, fingerprint: Optional[str] = None) -> None:
    """
    Open the Playwright inspector
    """
    from .sync_api import Camoufox

    with Camoufox(fingerprint=fingerprint, headless=False, env=environ, config={'showcursor': False}) as browser:
...
def server(fingerprint: Optional[str] = None) -> None:
    """
    Launch a Playwright server
    """
    from .server import launch_server

    launch_server(fingerprint=fingerprint)
